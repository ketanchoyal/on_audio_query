...
/** OnPodcastsQuery */
class OnPodcastsQuery : ViewModel() {
    ...
    fun queryPodcasts(
        ...
        if (call.argument<String>("path") != null) {
            val projection = songProjection() // TODO Implement custom podcast projection?
        ...
        viewModelScope.launch {
            ...
            // Empty list.
            var resultPodcastList = ArrayList<MutableMap<String, Any?>>()

            // We cannot "query" without permission so, just return a empty list.
            if (hasPermission) {
                // Start querying
                resultPodcastList = loadPodcasts()
            }

            //Flutter UI will start, but, information still loading
            result.success(resultPodcastList)
        }
    }

    //Loading in Background
    private suspend fun loadPodcasts(): ArrayList<MutableMap<String, Any?>> =
        withContext(Dispatchers.IO) {
            ...
            // Empty list.
            val podcastList: ArrayList<MutableMap<String, Any?>> = ArrayList()
